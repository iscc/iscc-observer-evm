...
#   filename:  https://testnet.iscc.id/api/openapi.json
#   timestamp: 2022-04-18T09:43:18+00:00
...
class API(BaseModel):
    openapi: Optional[str] = Field("/api/openapi.json", title="Openapi")
    version: Optional[str] = Field("0.1.0", title="Version")
    docs: Optional[str] = Field("/api/docs", title="Docs")


class Forecast(BaseModel):
    iscc_id: Optional[
        constr(regex=r"^ISCC:[A-Z2-7]{10,73}$", min_length=15, max_length=73)
    ] = Field(None, description="ISCC-ID", example="ISCC:MMAOHZYGQLBASTFM", title="Iscc Id")
    ...
    iscc_code: Optional[constr(regex=r"^[A-Z2-7]{10,73}$", min_length=15, max_length=73)] = Field(
        None,
...
class Head(BaseModel):
    ...
    block_hash: constr(max_length=255) = Field(..., description="Hash of block", title="Block Hash")
    tx_idx: int = Field(
        ...,
        description="Index of transaction within block including the ISCC-DECLARATION",
        title="Transaction Index",
    )
    tx_hash: constr(max_length=255) = Field(
        ...,
...
class RegistrationResponse(BaseModel):
    ...
    iscc_id: constr(regex=r"^ISCC:[A-Z2-7]{10,73}$", min_length=15, max_length=73) = Field(
        ..., description="Globally unique ISCC-ID", example="ISCC:MMAOHZYGQLBASTFM", title="Iscc Id"
    )
...
class Declaration(BaseModel):
    ...
    iscc_code: constr(regex=r"^[A-Z2-7:]{10,73}$", min_length=15, max_length=73) = Field(
        ...,
